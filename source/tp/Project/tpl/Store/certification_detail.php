   ...
   <table cellpadding="0" cellspacing="0" class="frame_form" width="100%">
	...
	<?php foreach($diy_attestation_list as $k=>$v){?>
        <tr>
            ...
            <td><div class="show"><?php if(strpos($v,'images')!==false) {?><img src="<?php echo  getAttachmentUrl($v)?>" width="120" height="120px"/><?php }else{ ?><?php echo $v; ?><?php } ?></div></td>
        </tr>
 <?php } ?>

		<tr>
            <td colspan="5" class="right-border" style="text-align:center">
                <span class="cb-enable approve-enable"><label class="cb-enable <if condition="$store_info['approve'] eq 1">selected</if>" data-id="<?php echo $_GET['id']; ?>"><span>认证通过</span><input type="radio" name="approve" value="1" /></label></span>
                <span class="cb-disable approve-disable"><label class="cb-disable <if condition="$store_info['approve'] eq 3">selected</if>" data-id="<?php echo $_GET['id']; ?>"><span>认证不通过</span><input type="radio" name="approve" value="3" /></label></span>
            </td>
        </tr>
