...
//          }
    ...
    private function scan($id, $openid = ''){
    	...
    	if($id>800000000){
			...
			$return[] = array('[店铺-'.$store['name'].']', $store['name'], getAttachmentUrl($store['logo']), $url);
    	
    	}elseif($id > 500000000 && $openid) {
    	//活动场景
    		$modelId = $id-500000000;
    		if($activity = M('Activity_recommend')->where(array('modelId'=>$modelId))->field("title,info,image,model,id,token")->find()){
    			 
    			$activity_url 	= $this->config['syn_domain']  ? rtrim($this->config['syn_domain'],'/').'/' : 'http://demo.pigcms.cn/';
    			if($activity['model'] == 'unitary'){
    				$url 	= $activity_url.'/index.php?g=Wap&m='.ucfirst($activity['model']).'&a=goods&token='.$activity['token'].'&unitaryid='.$modelId;
    			}else if($type == 'cutprice'){
    				$url 	= $activity_url.'/index.php?g=Wap&m='.ucfirst($activity['model']).'&a=goods&token='.$activity['token'].'&id='.$modelId;
    			}else{
    				$url 	= $activity_url.'/index.php?g=Wap&m='.ucfirst($activity['model']).'&a=index&token='.$activity['token'].'&id='.$modelId;
    			}
    		...
    		if ($user = D('User')->field('uid')->where(array('openid' => $openid))->find()){
    			...
    			$return[] = array('点击授权登录', '', $this->config['site_logo'], $this->config['wap_site_url'] . '/weixin_bind.php?qrcode_id=' . $id);
    			return array($return, 'news');
    		}
    	}
