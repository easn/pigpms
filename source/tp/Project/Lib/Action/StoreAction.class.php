...
class StoreAction extends BaseAction {
    ...
    public function certification_detail() {
        ...
        $certification_info = M('Certification')->where($where)->find();
    ...
    public function activityManage() {
        ...
        if (!empty($_GET['activity_type'])) {
            ...
            if (!in_array($selectModel, $_activityModel)) {
        ...
        if ($this->isPost()) {
            foreach ($_POST as $val) {
                ...
                if (!$activityRecommend) {
                    ...
                    $get_qid = $data['modelId'];
                    $qid = (int) $qid + (int) $get_qid;

                    /*
                      //加入活动二维码进入本地
                      //下载二维码图片 @simon
                      $local_file = D('Recognition')->download_img_from_weixin($qid);
                    ...
                    //存入 微信服务器端生成的临时二维码
                    $qcode = D('Recognition')->get_tmp_qrcode($qid);
                    if ($qcode['error_code'] == '0') {

                        $data['qcode'] = $qcode['ticket'];
                        $data['qcode_starttime'] = time();
                    }
    ...
    public function activityRecommend() {
        ...
        if (!empty($_GET)) {
            ...
            if (!in_array($_GET['activity_type'], array('999', ''))) {
    ...
    public function activityRecommendDel() {
        if ($this->isPost()) {
            ...
            $activity = M('Activity_recommend')->where(array('id' => array('in', $data)))->select();
            ...
            foreach ($activity as $k => $v) {
                if (empty($v['qcode']))
                    continue;
                if ($v['qcode']) {
                    unlink('./upload/' . $v['qcode']);
                    // 从又拍云服务器删除
                    $attachment_upload_type = C('config.attachment_upload_type');
                    if ($attachment_upload_type == '1') {
                        import('upyunUser', './source/class/upload/');
                        upyunUser::delete('/' . $v['qcode']);
                    }
                }
            }
        ...
        if (IS_POST) {
            ...
            for ($i = 0; $i < count($_POST['field-type']); $i++) {
                switch ($_POST['field-type'][$i]) {
                    case 'text':
                        $data['type'] = 'type=' . $_POST['field-type'][$i] . '&validate=required:true,' . $_POST['field-type-str'][$i] . ':true';
                        break;
                    case 'image':
