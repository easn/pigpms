...
if(empty($action)){
	...
	$cartList = D('')->field('`uc`.`pigcms_id`,`uc`.`product_id`,`uc`.`pro_num`,`uc`.`pro_price`,`uc`.`sku_id`,`uc`.`sku_data`, `uc`.`store_id`, `p`.`name`,`p`.`image`,`p`.`quantity`,`p`.`status`,`p`.`buyer_quota`')->table(array('User_cart'=>'uc','Product'=>'p'))->where($cart_where)->order('`pigcms_id` DESC')->select();
	...
	foreach($_POST['ids'] as $value){
		$now_cart = D('')->field('`uc`.*,`p`.`name`, `p`.`buyer_quota`,`p`.`quantity`,`p`.`status`, `p`.`weight`')->table(array('User_cart'=>'uc','Product'=>'p'))->where($cart_where." AND `uc`.`pigcms_id`='$value'")->find();
		if(empty($now_cart)){
		...
		if(!empty($now_cart['sku_id'])){
			...
			$product_sku = D('Product_sku')->field('`quantity`, `weight`')->where($condition_product_sku)->find();
			$quantity = $product_sku['quantity'];
	...
	foreach($cartList as $value){
        $product_info = $database_product->field('is_fx,product_id,store_id,original_product_id,supplier_id,wholesale_product_id')->where(array('product_id' => $value['product_id']))->find();
    ...
    if (!empty($suppliers)) { //修改订单，设置分销商
        ...
        if (!empty($suppliers) && (count($tmp_suppliers) > 1)) {
            $data['is_fx'] = 1;
        }
		if (!empty($type)) {
			$data['type'] = $type;
