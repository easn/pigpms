...
if (IS_GET && $_GET['a'] == 'index') {
    $subscribe_store_model = M('Subscribe_store');

    $store = $_SESSION['wap_drp_store'];
    //粉丝总数
    $where = array();
    $where['u.store_id'] = $store['store_id'];
    $fans = $subscribe_store_model->getFansCount($where);
    //今日新增粉丝
    $start_time = strtotime(date('Y-m-d') . ' 00:00:00');
    $end_time = strtotime(date('Y-m-d') . ' 23:59:59');
    $where = array();
    $where['u.store_id'] = $store['store_id'];
    $where['_string'] = "u.subscribe_time >= '" . $start_time . "' AND u.subscribe_time <= '" . $end_time . "'";
    $today_fans = $subscribe_store_model->getFansCount($where);
    //昨日新增粉丝
    ...
    $where['u.store_id'] = $store['store_id'];
    $where['_string'] = "u.subscribe_time >= '" . $start_time . "' AND u.subscribe_time <= '" . $end_time . "'";
    $yesterday_fans = $subscribe_store_model->getFansCount($where);
    ...
    if ($date == 'today') { //今日
        $start_time = strtotime(date('Y-m-d') . ' 00:00:00');
        $end_time = strtotime(date('Y-m-d') . ' 23:59:59');
        $where['_string'] = "subscribe_time >= '" . $start_time . "' AND subscribe_time <= '" . $end_time . "'";
    } else if ($date == 'yesterday') { //昨日
        $start_time = strtotime(date("Y-m-d", strtotime("-1 day")) . ' 00:00:00');
        $end_time = strtotime(date("Y-m-d", strtotime("-1 day")) . ' 23:59:59');
        $where['_string'] = "subscribe_time >= '" . $start_time . "' AND subscribe_time <= '" . $end_time . "'";
    }
    $fans_count = $subscribe_store_model->getFansCount($where);
    //import('source.class.user_page');
    //$page = new Page($fans_count, 20);
    //$fans = $subscribe_store_model->getFans($where, $page->firstRow, $page->listRows);
    $fans = D('Subscribe_store')->where($where)->select();
    ...
    $fans = $subscribe_store_model->getFans($where, $page->firstRow, $page->listRows, $order);
    $html = '';
