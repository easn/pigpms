...
switch($action){
	...
	case 'nearactive':
		...
		$artivce_list = D('')->table("Activity_recommend as ar")
							...
							->where("`s`.`status`='1' AND and `s`.`public_display`=1 and s.drp_supplier_id = '0' and is_rec=1")
							->group ( '`ar`.`id`' )
							->order ( "`juli` asc" )
							->limit(4)
							->select();		
	...
	case 'neargoods':
		...
		$goods_list = D('')->table("Product as p")
							...
							->where("`s`.`status`='1' AND `S`.`public_display`='1' AND s.drp_supplier_id = '0' and `p`.`is_recommend` = 1 AND p.supplier_id = 0")
							->group ( 'p.store_id' )
							->order ( "`juli` asc"  )
							->limit(4)
							->select();	
