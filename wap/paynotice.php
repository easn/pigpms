        ...
        if ($order_peerpay && $order_peerpay['status'] == '0') {
            ...
            if ($result) {
                ...
                if (!empty($order) && $order['total'] <= $pay_money) {
                    ...
                    $order_info['order_param']['third_id'] = '';
    ...
    if ($payInfo['err_code'] === 0) {
        ...
        if ($nowOrder && $nowOrder['status'] == 1) {
            ...
            if (D('Order')->where($condition_order)->data($data_order)->save()) {
                    ...
                    if($nowOrder['activity_data'] != ''){
                        ...
                        if ($bak && isset($bak['from'])) {
                            ...
                            $postData = array(
                                ...
                                'from'  => $bak['from'],
                                'transactionid' => $data_order['third_id'],
                                'model' => 'Nothing',
                                'token' => $bak['token'],
                            );
                ...
                if ($drp_level > 3 && ($max_store_drp_level > 3 || empty($max_store_drp_level))) {
                ...
                foreach ($orderProductList as $value) {
                    ...
                    if (!empty($value['supplier_id'])) { //非自营商品
                        ...
                        } else if (!empty($product['is_wholesale'])) { //允许批发商品
                            sync_sku($value['product_id'], $value['product_id'], $value['sku_data'], $value['pro_num'], false);
                        }
                    }
                }
...
function wholesale_supplier_pay($user_order_id, $supplier_id, $seller_id, $products, $is_seller)
{
    ...
    $seller_postage = !empty($fx_postages[$seller_id]) ? $fx_postages[$seller_id] : 0; //经销商运费
    //订单总金额
    $total = $sub_total + $postage + $seller_postage;
    //订单成本总金额
    $cost_total = $cost_sub_total + $postage;
    $fx_order_no = date('YmdHis', $_SERVER['REQUEST_TIME']) . mt_rand(100000, 999999); //分销订单号
    $data = array(
 ...
 */
function sync_sku($product_id, $original_product_id, $sku_data, $quantity, $update_sales = true)
{
    $where = array();
    //供货商的商品或从供货商分销来的商品
