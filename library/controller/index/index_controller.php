...
class index_controller extends base_controller{
	...
	public function index() {
		...
		$where = "p.status = 1 and p.public_display=1 and p.quantity > 0 and 'p.supplier_id'=0 and p.public_display=1";
		$order_by_field = "sales";
		$order_by_method = "desc";

		//默认
		$hot_products['product'] = M('Product')->getSellingAndDistance($where, $order_by_field, $order_by_method, 0, 10);
		...
		if($cookie_location['long']) {
			...
			$nearshops	= M('Store')->getlist(array('status'=>1,'public_display'=>1),$orderby,10);
		}
	...
	function ajax_hot_product() {
		...
		if (is_array($catid_arr)) {
			foreach ($catid_arr as $k1 => $v1) {
				...
				$where1 = "p.status = 1 and p.public_display=1 and p.quantity > 0 and 'p.supplier_id'=0 and (p.category_id ='".$v1."' or p.category_fid ='".$v1."') ";
