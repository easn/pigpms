...
class order_controller extends base_controller {
    ...
    public function __construct() {
        ...
        $exclude = array(
            'goods_load', 'comments_load', 'load', 'delivery_load', 'physical', 'package_product_phy', 'detail', 'create_package'
        ); //排除验证方法列表
...
// 					}
	...
	private function _order_rights_detail() {
        ...
        $tmp_orders = $order->getAllOrders($where, $orderby);
        $orders = array();
        foreach ($tmp_orders as $tmp_order) {
            $products = $order_product->getProducts($tmp_order['order_id']);
            $tmp_order['products'] = $products;
...
//或者include 'PHPExcel/Writer/Excel5.php'; 用于输出.xls的
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('Y1', '订单地址');
        ...
        foreach ($orders as $k => $v) {
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('Y' . ($k + 2), $address_str);
        }
    ...
    public function order_download_csv2() {
        $order = M('Order');
        $order_product = M('Order_product');
        $user = M('User');
        $where = array();
...
$objWriter = new PHPExcel_Writer_Excel5($objPHPExcel); //非2007格式
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('G1', '收货联系电话');
		//二次开发
		$objPHPExcel->getActiveSheet()->setCellValue('H1', '身份证号');
        $objPHPExcel->getActiveSheet()->setCellValue('I1', '省');
        $objPHPExcel->getActiveSheet()->setCellValue('J1', '市');
        $objPHPExcel->getActiveSheet()->setCellValue('K1', '区');
        $objPHPExcel->getActiveSheet()->setCellValue('L1', '详细');
		//以上二次开发
        $objPHPExcel->getActiveSheet()->setCellValue('M1', '支付方式');
        $objPHPExcel->getActiveSheet()->setCellValue('N1', '订单类型');
        $objPHPExcel->getActiveSheet()->setCellValue('O1', '订单时间');
        $objPHPExcel->getActiveSheet()->setCellValue('P1', '付款时间');
        $objPHPExcel->getActiveSheet()->setCellValue('Q1', '发货时间');
        $objPHPExcel->getActiveSheet()->setCellValue('R1', '收货时间');
        $objPHPExcel->getActiveSheet()->setCellValue('S1', '取消时间');
        $objPHPExcel->getActiveSheet()->setCellValue('T1', '订单完成时间');
        $objPHPExcel->getActiveSheet()->setCellValue('U1', '退款时间');
        $objPHPExcel->getActiveSheet()->setCellValue('V1', '买家留言');
        $objPHPExcel->getActiveSheet()->setCellValue('W1', '备注');
        $objPHPExcel->getActiveSheet()->setCellValue('X1', '实际付款金额');
        $objPHPExcel->getActiveSheet()->setCellValue('Y1', '订单取消方式');
        $objPHPExcel->getActiveSheet()->setCellValue('Z1', '商品供货商');
        $objPHPExcel->getActiveSheet()->setCellValue('AA1', '是否对账');
        $objPHPExcel->getActiveSheet()->setCellValue('AB1', '商品金额');
        $objPHPExcel->getActiveSheet()->setCellValue('AC1', '订单金额');


        foreach ($orders as $k => $v) {			
            $tmp_info = '';
            foreach ($v['products'] as $val) {
				$skus=unserialize($val['sku_data']);
            	$data="";
            	foreach($skus as $sku){
            		$data=$data.$sku['name'].$sku['value']."  ";
            	}
                $tmp_info .= "商品名称：" . $val['name'] . "  商品数量：" . $val['pro_num'] . "  商品价格：" . $val['pro_price'] . " 商品规格:" .$data . "\n";
				$comment=unserialize($val['comment']);
				for($i=0;$i<sizeof($comment);$i++){
				if($comment[$i]['name']=="身份证号"){
						$cardid=$comment[$i]['value'];
					}
				}
            }
		    $address= unserialize($v['address']);
			...
			//二次开发
			$objPHPExcel->getActiveSheet()->setCellValue('H' . ($k + 2), ' '.$cardid);
            $objPHPExcel->getActiveSheet()->setCellValue('I' . ($k + 2), $address['province']);
            $objPHPExcel->getActiveSheet()->setCellValue('J' . ($k + 2), $address['city']);
            $objPHPExcel->getActiveSheet()->setCellValue('K' . ($k + 2), $address['area']);
            $objPHPExcel->getActiveSheet()->setCellValue('L' . ($k + 2), $address['address']);
             //以上二次开发
			$objPHPExcel->getActiveSheet()->setCellValue('M' . ($k + 2), $payment_method[$v['payment_method']]);
            $objPHPExcel->getActiveSheet()->setCellValue('N' . ($k + 2), $v['type'] == 0 ? '普通' : ($v['type'] == 1 ? '代付' : ($v['type'] == 2 ? '送礼' : '分销')));
            $objPHPExcel->getActiveSheet()->setCellValue('O' . ($k + 2), $v['add_time'] ? date('Y-m-d H:i:s', $v['add_time']) : '');
            $objPHPExcel->getActiveSheet()->setCellValue('P' . ($k + 2), $v['paid_time'] ? date('Y-m-d H:i:s', $v['paid_time']) : '');
            $objPHPExcel->getActiveSheet()->setCellValue('Q' . ($k + 2), $v['sent_time'] ? date('Y-m-d H:i:s', $v['sent_time']) : '');
            $objPHPExcel->getActiveSheet()->setCellValue('R' . ($k + 2), $v['delivery_time'] ? date('Y-m-d H:i:s', $v['delivery_time']) : '');
            $objPHPExcel->getActiveSheet()->setCellValue('S' . ($k + 2), $v['cancel_time'] ? date('Y-m-d H:i:s', $v['cancel_time']) : '');
            $objPHPExcel->getActiveSheet()->setCellValue('T' . ($k + 2), $v['complate_time'] ? date('Y-m-d H:i:s', $v['complate_time']) : '');
            $objPHPExcel->getActiveSheet()->setCellValue('U' . ($k + 2), $v['refund_time'] ? date('Y-m-d H:i:s', $v['refund_time']) : '');
            $objPHPExcel->getActiveSheet()->setCellValue('V' . ($k + 2), $v['comment']);
            $objPHPExcel->getActiveSheet()->setCellValue('W' . ($k + 2), $v['bak']);
            $objPHPExcel->getActiveSheet()->setCellValue('X' . ($k + 2), $v['pay_money']);
            $objPHPExcel->getActiveSheet()->setCellValue('Y' . ($k + 2), $v['cancel_method'] == 0 ? '正常（过期自动取消）' : ($v['cancel_method'] == 1 ? '卖家手动取消' : '买家手动取消'));
            $objPHPExcel->getActiveSheet()->setCellValue('Z' . ($k + 2), $store_info['name']);
            $objPHPExcel->getActiveSheet()->setCellValue('AA' . ($k + 2), $v['is_check'] == 1 ? '未对账' : '已对帐');
            $objPHPExcel->getActiveSheet()->setCellValue('AB' . ($k + 2), $v['sub_total']);
            $objPHPExcel->getActiveSheet()->setCellValue('AC' . ($k + 2), $v['total']);
