...
class trade_controller extends base_controller{
    ...
    public function __construct() {
        ...
        $exclude = array(
            'goods_load','comments_load','load','delivery_load'
        );//排除验证方法列表
	...
	private function _income_content()
	{
		...
		$where = array();
		$where['store_id'] = $this->store_session['store_id'];
		$where['type'] = array('in', array(1,5,6));
	...
	private function _wholesale_content()
	{
		...
		foreach ($orders as &$order) {
			$profit = D('Financial_record')->where(array('order_id' => $order['order_id'], 'type' => 6))->sum('income');
	...
	private function _drp_content()
	{
		...
		$profit_sub_total = 0;
		foreach ($orders as &$order) {
			$profit = D('Financial_record')->where(array('order_id' => $order['order_id'], 'type' => 5))->sum('income');
			$order['profit'] = number_format($profit, 2, '.', ''); //利润
			...
			$seller = D('Store')->field('name')->where(array('store_id' => $fx_order['store_id']))->find();
			$order['seller'] = $seller['name'];
			if (empty($order['seller']) && empty($order['user_order_id'])) {
				$order['seller'] = '本店';
			}
