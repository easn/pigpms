            ...
            switch ($value['field_type']) {
                ...
                case 'attention_collect':
					...
					*/
                    ...
                    $fieldHtml['html'] .= '<div class="ft-links custom-attention_collect"><a href="javascript:userCollect(' . $store_id . ', 2)"target="_blank" class="js-shouchang">收藏店铺(<span>' . ($store_info['collect'] ? $store_info['collect'] : 0) . '</span>)</a><a href="javascript:userAttention(' . $store_id. ', 2)" class="js-guanzhu">关注店铺(<span>' . ($store_info['attention_num'] ? $store_info['attention_num']: 0) . '</span>)</a></div>';
                    break;
                    ...
                    foreach ($fieldContent as $v) {
                        if ($v['type'] == 'link') {
                            ...
                            foreach ($page_list as $jv) {
                                ...
                                $fieldHtml['html'] .= '<li><a class="clearfix relative arrow-right" href="' . $url . '" target="_blank"><span class="custom-nav-title">' . htmlspecialchars_decode($jv['page_name']) . '</span></a></li>';
                            }
                            ...
                            foreach ($product_list as $jv) {
                                ...
                                $fieldHtml['html'] .= '<li><a class="clearfix relative arrow-right" href="' . $url . '" target="_blank"><span class="custom-nav-title">' . htmlspecialchars_decode($jv['name']) . '</span></a></li>';
                            }
                        }
                    }
                    $fieldHtml['html'] .= '</ul>';
                    ...
                    if (!empty($fieldContent['goods'])) {
                        if ($fieldContent['size'] == 0) {
                            ...
                            foreach ($fieldContent['goods'] as $key => $value) {
                                ...
                                if ($drp_level > 0 && !empty($product['unified_price_setting'])) {
                ...
                case 'tpl_shop':
                    ...
                    //全部已经上架的商品
                    $count = M('Product')->getSellingTotal(array('store_id' => $store_id));
                            ...
                            foreach ($v as $val) {
                                if (!empty($val['face_money'])) {
                                    ...
                                    if ($coupons) {
                                        ...
                                        $description = $coupons['description'] ? msubstr($coupons['description'], 0, 7) : '&nbsp;';
                                        $fieldHtml['html'] .= '<li>  <a href="/wap/store_coupon.php?id=' . $store_id . '"><div class="custom-coupon-price"><span>￥</span>' . $coupons['face_money'] . '</div><div class="custom-coupon-desc">' . $description . '</div>  </a> </li>';
                                    }
                                }
                            }
                        }
...
//                    //全部start
                    ...
                    foreach ($fieldContent['goods_group1'] as $k => $v) {
                        $fieldHtml['html'] .= '<p class="custom-tag-list-title" id="anchor-1439862749815c193-10924959">' . $v["title"] . '</p>';
                    ...
                    if (!empty($fieldContent['goods'])) {
                        ...
                        if ($fieldContent['size'] == 0) {
                            ...
                            foreach ($fieldContent['goods'] as $key => $value) {
                                ...
                                $product_list = M('Product')->getSelling(array('product_id' => array('in', $product_arr)), '', '', 0, 1000);
                                ...
                                if (empty($product_list)) {
                                    ...
                                    foreach ($product_list as $k => $v) {
                                        ...
                                        $fieldHtml['html'].='<li class="goods-card goods-list big-pic ' . ($fieldContent['size_type'] == 2 ? 'normal' : 'card') . '">';
                                        $fieldHtml['html'].='<a href="' . $url . '" class="js-goods link clearfix" target="_blank" title="' . htmlspecialchars_decode($v['name']) . '">';
                                        $fieldHtml['html'].='<div class="photo-block" style="height:auto;">';
                            ...
                            if ($fieldContent['size_type'] != 1) {
                                ...
                                foreach ($fieldContent['goods'] as $key => $value) {
                                    ...
                                    foreach ($product_list as $key => $product) {
                                        ...
                                        $fieldHtml['html'].='<li class="goods-card goods-list small-pic waterfall" style="width:155px;">';
                                        $fieldHtml['html'].='<a href="' . $url . '" class="js-goods link clearfix" target="_blank" title="' . htmlspecialchars_decode($product['name']) . '">';
                                        $fieldHtml['html'].='<div class="photo-block">';
                                        $fieldHtml['html'].='<img class="goods-photo js-goods-lazy" src="' . getAttachmentUrl($product['image']) . '" style="display:inline;">';
                                        $fieldHtml['html'].='</div>';
                                        $fieldHtml['html'].='<div class="info clearfix ' . ($fieldContent['show_title'] == 1 ? 'info-title ' : 'info-no-title ') . ($fieldContent['price'] == 1 ? 'info-price ' : 'info-no-price ') . 'btn' . $fieldContent['buy_btn_type'] . ($fieldContent['show_title'] == 1 || $fieldContent['price'] == 1 ? '' : ' hide') . '">';
