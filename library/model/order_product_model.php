...
class order_product_model extends base_model{
	 ...
	 * 查看用户对产品是否未评论
	 */
	public function isComment($product_id, $is_pc = true,$order_id = "") {
		if (empty($product_id)) {
			return false;
		}
		$wheres = "";
    ...
    {
        $sql = "SELECT DISTINCT op.*,fop.cost_price AS price FROM " . option('system.DB_PREFIX') . "order_product op," . option('system.DB_PREFIX') . "fx_order_product fop WHERE op.product_id = fop.source_product_id AND op.order_id = '" . $order_id . "' AND fop.fx_order_id = '" . $fx_order_id . "'";
        if (!$supplier) {
            $sql .= ' AND op.is_fx = 1';
        }
