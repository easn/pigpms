...
class product_model extends base_model
{
    ...
    // 仓库中的商品
    public function getSoldout($where, $order_by_field, $order_by_method, $limit="0", $offset="0")
    {
        if (!empty($order_by_field) && !empty($order_by_method)) {
            $order = $order_by_field . ' ' . strtoupper($order_by_method);
            if ($order_by_field == 'sort') {
    ...
    public function getSearchGroupGoodList($key, $page, $store_id, $seller_id = 0, $drp_level = 0, $drp_diy_store = 1)
    {
        $database_product = D('Product');
        $condition_product['store_id'] = $store_id;
        if (empty($drp_diy_store)) {
            $condition_product['is_fx'] = 1;
            $condition_product['unified_price_setting'] = 1;
        }
        ...
        if ($count > 0) {
            ...
            foreach ($product_list as &$value) {
                ...
                if (empty($drp_diy_store) && $drp_level > 0) {
                    if ($drp_level > 3) {
                        $drp_level = 3;
                    }
