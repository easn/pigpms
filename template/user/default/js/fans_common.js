...
$(function(){
    ...
    $('.ui-nav > ul > li > a').live('click', function(){
        ...
        load_page('.app__content', load_url, {'page': page_content, 'status': status, 'p': page,  'start_time': start_time, 'stop_time': stop_time, 'nickname': nickname}, '', function(){
            ...
            if (status != 0 && $('.ui-nav > ul > li >.status-' + status).length == 0) {
    ...
    $('.js-assign-physical').live('click', function(){
        ...
        window.assign_physical = $.post(package_assign_url, {'order_id': order_id}, function(data){
            ...
            html += '       <div class="modal-header ">';
            html += '           <a class="close" data-dismiss="modal">×</a>';
            html += '           <h3 class="title">订单商品分配到门店</h3>';
            html += '       </div>';
            html += '       <div class="modal-body">';
            html += '           <table class="ui-table">';
            html += '               <thead>';
            html += '                   <tr>';
            html += '                       <th class="text-right cell-5">';
            html += '                           <input type="checkbox" checked="true" value="1" class="js-check-all hide" />';
            html += '                       </th>';
            html += '                       <th class="cell-35">商品</th>';
            html += '                       <th class="cell-10">数量</th>';
            html += '                       <th class="cell-30 text-right">分配门店[库存数]</th>';
            html += '                   </tr>';
            html += '               </thead>';
            html += '               <tbody>';
            if (data.products != '' && data.products.length > 0) {
                for (i in data.products) {
                    html += '                   <tr>';
                    html += '                       <td class="text-right">';
                    html += '                           <input type="checkbox" class="js-check-item hide" checked="true" value="' + data.products[i]['product_id'] + '" />';
                    html += '                           <input type="hidden" value="' + data.products[i]['product_id'] + '" class="js-hidden-product" />';
                    html += '                           <input type="hidden" value="' + data.products[i]['sku_id'] + '" class="js-hidden-sku" />';
                    html += '                           <input type="hidden" value="' + data.products[i]['order_product_id'] + '" class="js-hidden-op_id" />';
                    html += '                       </td>';
                    html += '                       <td>';
                    html += '                           <div>';
                    html += '                               <a href="" class="new-window" target="_blank">' + data.products[i]['name'] + '</a>';
                    ...
                    html += '                       <td>' + data.products[i]['pro_num'] + '</td>';
                    html += '                       <td class="text-right">';
                    if (data.products[i].physical.length > 0) {
                        html += '                           <select class="js-select-all" style="width: 120px;">';
                        html += '                              <option value="0" selected="selected">'+'--不分配--'+'</option>';
                        for (k in data.products[i].physical) {
                            html += '                          <option value="'+data.products[i].physical[k].physical_id+'">'+data.products[i].physical[k].name+'['+data.products[i].physical[k].quantity+']'+'</option>';
                        }
                        html += '                           </select>';
                    } else {
                        html += '                           <select class="js-select-all" style="width: 120px;"><option value="0" selected="selected">门店无货</option></select>';
                    }
                    html += '                       </td>';
                    html += '                   </tr>';
                }
    ...
    $('.js-save-assign').live('click', function(){
        ...
        if (flag) {
            $('.js-hidden-product').each(function(){
                var self = $(this);
                var itemObj = {product_id:0, sku_id:0, order_product_id: 0,physical_id:0};
                itemObj.product_id = self.val();
                itemObj.sku_id = self.parent().find("input.js-hidden-sku").val();
                itemObj.order_product_id = self.parent().find("input.js-hidden-op_id").val();
                itemObj.physical_id = self.parents("tr:first").find("select option:selected").val();

                if (itemObj.physical_id != 0) {
                    package_item.push(itemObj);
                }
            })

            if (package_item.length < 1) {
                $('.notifications').html('<div class="alert in fade alert-error">' + '至少需要一个门店' + '</div>');
                t = setTimeout('msg_hide()', 3000);
                return false;
            }

            $.post(package_assign_save_url, {'order_id': order_id, 'package_item': package_item}, function(data) {
                if (!data.err_code) {
                    $('.notifications').html('<div class="alert in fade alert-success">' + data.err_msg + '</div>');
                    $('.modal').animate({'margin-top': '-' + ($(window).scrollTop() + $(window).height()) + 'px'}, "slow",function(){
                        $('.modal-backdrop,.modal').remove();
    ...
    $('.js-save').live('click', function(){
        ...
        if (flag) {
            ...
            $.post(create_package_url, {'order_id': order_id, 'express_id': express_id, 'express_company': express_company, 'express_no': express_no, 'products': products.toString(), 'order_products': order_products.toString(), 'sku_data': sku_data.toString()}, function(data) {
                if (!data.err_code) {
                    $('.notifications').html('<div class="alert in fade alert-success">' + data.err_msg + '</div>');
                    $('.modal').animate({'margin-top': '-' + ($(window).scrollTop() + $(window).height()) + 'px'}, "slow",function(){
                        $('.modal-backdrop,.modal').remove();
