	...
	function location_page(mark,dom){
		...
		switch(mark_arr[0]){
			...
			case '#notice_list':
				...
				load_page('.app__content', load_url,{page:'notice_list'}, '');
				break;				
			case '#physical_store':
				$('.js-app-nav.list').addClass('active').siblings('.js-app-nav').removeClass('active');
    ...
    $('.js-btn-submit').live('click', function(){
		...
		*/
        if (!$('.error').length) {
            ...
            $.post(store_setting_url, {'name': name, 'logo': logo, 'intro': intro, 'linkman': linkman, 'qq': qq, 'mobile' : mobile, 'open_service': open_service,"is_show_drp_tel":is_show_drp_tel}, function(data){
                if (!data.err_code) {
