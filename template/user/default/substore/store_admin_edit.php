...
if (empty($methodList['goods'])) {
	$methodList['goods'] = array();
}

if (empty($methodList['order'])) {
	$methodList['order'] = array();
}

if (empty($methodList['trade'])) {
	$methodList['trade'] = array();
}
?>
<form class="form-horizontal" style="">
    ...
    <div class="control-group">
        ...
        <div class="controls js-goods-tip">
            ...
            <label class="checkbox inline">
                 <input type="checkbox" class="js-not-autoup" name="goods[]" <?php echo in_array('checkoutProduct', $methodList['goods']) ? 'checked' : '';?> value="checkoutProduct">商品导出
            </label>
            <label class="checkbox inline">
...
<script>
    $(function(){
        ...
        $('#sub-button').click(function(){
            ...
            //     $("input[name=goods_control]").after('<p class="help-block error-message">必须选择一个商品菜单</p>');
            // }else{
            ...
            //     $("input[name=order_control]").next('.error-message').remove();
            //     $("input[name=order_control]").after('<p class="help-block error-message">必须选择一个订单菜单</p>');
            // }else{
            ...
            // 订单、商品、物流选项至少需要一个值
            /*if ($(".js-order-tip").find("input:checked").length == 0) {
                $(".js-order-tip").find(".error-message").remove();
                $(".js-order-tip").append('<p class="help-block error-message">至少需要一个订单菜单</p>');
            } else {
                $(".js-order-tip").find(".error-message").remove();
            }

            if ($(".js-goods-tip").find("input:checked").length == 0) {
                $(".js-goods-tip").find(".error-message").remove();
                $(".js-goods-tip").append('<p class="help-block error-message">至少需要一个商品菜单</p>');
            } else {
                $(".js-goods-tip").find(".error-message").remove();
            }

            if ($(".js-express-tip").find("input:checked").length == 0) {
                $(".js-express-tip").find(".error-message").remove();
                $(".js-express-tip").append('<p class="help-block error-message">至少需要一个物流菜单</p>');
            } else {
                $(".js-express-tip").find(".error-message").remove();
            }*/
