    ...
    <div id="js-overview" class="ui-overview">
        ...
        <div class="overview-group">
            <div class="overview-group-inner">
                ...
                <?php } else { ?>
                    <a class="h4" href="<?php dourl('fx:goods'); ?>"><?php echo $fx_product_total; ?></a>
                    <span class="h5">分销商品</span>
                <?php } ?>
            </div>
        </div>
    </div>
		...
		<div class="ui-block-body">
			<div class="js-body" >
				<table class="ui-table">
					<thead>
						<tr>
							<th class="cell-12">粉丝名称</th>
							<th class="cell-25" style="color:#07d; text-align:center">关注时间</th>
							<th class="cell-12" style="text-align:center">来　　源</th>
						</tr>
					</thead>
					<tbody class="js-list-tbody">
						<?php if (!empty($fan_list)) { ?>
						<?php foreach ($fan_list as $v) { ?>
						<tr>
							<td><a href="javascript:void(0)" class="new-window"><?php echo $user_list[$v['uid']]['nickname']; ?></a></td>
							<td style="text-align:center"><?php echo date('Y-m-d H:i:s',$v['subscribe_time'])?></td>
							<td style="text-align:center"><?php echo $v['open_id']?'关注店铺':'访问店铺'?></td>
						</tr>
						<?php } ?>
						<?php }else{ ?>
						<tr>
