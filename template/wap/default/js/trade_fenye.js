		...
		$.ajax({
			...
			success:function(result){
				...
				if(result.err_code){
					...
					if(result.err_msg.count == 0) {
						$(".s_empty").removeClass("hide").show();
						return true;
					}
					if(result.err_msg.list.length > 0){
						...
						for(var i in result.err_msg.list){
							...
							for(var ii in order.order_product_list) {
								...
								if (order.is_return && order_product.return_status != "2" && order_product.is_present == "0") {
							...
							str+='<div class="opt-btn">';
							if(order.status=='3') {//alert(order.order_no)
								str+='<a class="btn btn-green btn-in-order-list js-delivery" href="javascript:" data-order_no="'+order.order_no_txt+'" style="width:auto;">确认收货</a>';
							}	
							if(order.status > 2 && order.is_assigned == 2) {
								str+='<a class="btn btn-green btn-in-order-list" href="./my_package.php?order_id='+order.order_id+' style="width:auto;">配送详情</a>';
							}
							if(order.status<2) {
			...
			error:function(){
				$('.wx_loading2').hide();
				$('.empty-list').show();
				motify.log('商品读取失败，<br/>请刷新页面重试',0);
			}
		});
	}
