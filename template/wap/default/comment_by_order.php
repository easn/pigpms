...
<script type="text/javascript" src="<?php echo TPL_URL;?>js/comment_by_order.js?time=<?php echo time();?>"></script>
					...
					<div class="shop_pingjia">  
						...
						>
								...
								<ul>
									...
									<li class="gray">
										<div class="appraise_li-list_top_icon bumanyi">
											<input type="radio" class="ui-checkbox" <?php if($product['score']<=2) {?>checked="checked" <?php }?> value="1" name="manyi<?php echo $product['product_id']?>" />
							...
							<div class="textarea">
								...
								<?php }else {?>
								<textarea  name='contents<?php echo $product['product_id'];?>'  placeholder="请写下对宝贝的感受吧，对他人帮自己很大哦！" cols="" rows="" class="form_textarea contents<?php echo $product['product_id'];?>"></textarea>
								<?php }?>
							</div>
							...
							<?php if($product['return_status']==0){?>
								<?php if(count($order_product_comment['comment_list'][$product['product_id']])) {}else{?>
									<div style="width:auto;line-height:22px;background:#fff">
										<button class="form_button js_save" js_submit_proid = "<?php echo $product['product_id'];?>" style="width:auto;line-height:22px; float:right;background:#00BB88;height:34px;color:#fff;padding:7px;">发表评论</button>
									</div>
								<?php }}?>
							<?php }?>
